...
<section class="page page-recherche">
  ...
  <!-- FORMULAIRE -->
  <form method="GET" action="<?= BASE_URL ?>index.php" class="card search-card">
    ...
    <input
      ...
      min="1"
      max="5"
      value="<?= htmlspecialchars($_GET['passagers'] ?? 1) ?>"
      required
    >
    ...
    <input
      ...
      min="0"
      max="5"
      value="<?= htmlspecialchars($_GET['note_min'] ?? '') ?>"
    >
      ...
      <div class="trajets-grid">
        ...
        <?php foreach ($resultats as $trajet): ?>
          ...
          <div class="trajet-card">
            ...
            <p>
              👥 <?= (int)$trajet['places_restantes'] ?> places restantes
            </p>

            <a
              class="btn btn-wide"
              href="<?= BASE_URL ?>index.php?url=details&id=<?= (int)$trajet['id'] ?>"
            >
              Détails
            </a>
    ...
    <?php else: ?>
      ...
      <?php if (!empty($prochain)): ?>
        ...
        <div class="msg-info">
          ...
          <strong>
            <?= htmlspecialchars($prochain['date_depart']) ?>
          </strong>

          à
          <?= htmlspecialchars($prochain['heure_depart']) ?>

          —

          <a
            href="<?= BASE_URL ?>index.php?url=details&id=<?= (int)$prochain['id'] ?>"
          >
            Voir
          </a>
